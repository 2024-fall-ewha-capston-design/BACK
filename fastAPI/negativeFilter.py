...
prompt = ChatPromptTemplate.from_messages([
    ("system",
     ...
     "- 메시지 '내일 면접 떨려요'는 의미적으로 '취업 준비'라는 키워드와 일치해야 해. 예를 들어, '자소서 작성했나요?', '면접관', '인적성 어떡하죠'와 같은 인사말도 '취업 준비' 키워드와 일치해야 해.\n"
...
def perform_negative_analysis(messages: List[str], participant_keywords: Dict[str, List[Dict[str, str]]]) -> List[Dict[str, str]]:
    try:
        # GPT로 분석 요청
        result = chain.invoke({
            "messages": messages,
